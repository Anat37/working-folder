...
andObject3::andObject3() {}

andObject3::andObject3(Object3* f, Object3* s)
	:_first(f)
	, _second(s) {
...
ld andObject3::isIntercectLine(const Line& ray, SurfacedPoint3& ptr) const {
...
ld andObject3::getMax(const int dim) const {
	return _maxv[dim];
}

bool andObject3::isPhantomLighter() const {
	return (_first->isPhantomLighter() || _second->isPhantomLighter());
}

std::vector<PhantomLighter* > andObject3::getPhantomLighter(Lighter* light) const {
