...
Color* Scene::render(Screen screen, Viewer viewer) {
	...
	for (int x = 0; x < sizeX; ++x)
		...
		ptr[x + y*sizeX] = getClosestIntercection(ray).color;
	}
	return ptr;
}
...
Color Scene::getLighting(Point3 p) {
	Color total(0,0,0,0);
	for (unsigned int i = 0; i < _lighters.size(); ++i) {
		Line ray{p, _lighters[i]->getLocation() - p};
...
ColoredPoint3 Scene::getClosestIntercection(Line ray) {
